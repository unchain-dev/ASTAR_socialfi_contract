...
use crate::astar_socialfi::AstarSocialfi;
...
impl AstarSocialfi {
    ...
    // 全ての投稿から指定の最新度の投稿を取得
    pub fn get_all_posts_fn(&self, num: u128) -> Vec<Post> {
        // 返す投稿のリスト用のリストを生成
    ...
    // 個人の投稿を取得
    pub fn get_user_posts_fn(&self, num: u128, account_id: AccountId) -> Vec<Post> {
        // 返す投稿のリスト用のリストを生成
        let mut post_list: Vec<Post> = Vec::new();
    ...
    pub fn get_total_likes_fn(&self, account_id: AccountId) -> u128 {
        ...
        for id in post_id_list {
            let likes_of_post = self.post_map.get(&id).unwrap().total_likes;
